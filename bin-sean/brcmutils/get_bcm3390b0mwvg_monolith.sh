...
function show_help()
{
        echo "Usage: get_bcm3390b0mwvg_monolith.sh"
...
if [ "$cm_filename" != "" ]
then
    get_monolith.sh -t mwvg -d $target_date -b $branch -x $cxc_version -f $cm_filename
else
   get_monolith.sh -t mwvg -d $target_date -b $branch -x $cxc_version
 fi
