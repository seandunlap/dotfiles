...
function show_help()
{
        echo "Usage: get_bcm3390b0dcm_monolith.sh"
...
if [ "$cm_filename" != "" ]
then
    get_monolith.sh -t dcm -d $target_date -b $branch -f $cm_filename
else
   get_monolith.sh -t dcm -d $target_date -b $branch
 fi
